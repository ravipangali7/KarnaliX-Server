"""
Super views for Client Requests (Deposit/Withdraw).
"""
...
def deposit_list(request):
    """Get all deposit requests for hierarchy users."""
    user = request.user
...
def withdraw_list(request):
    """Get all withdraw requests for hierarchy users."""
    user = request.user
    hierarchy_users = get_hierarchy_users(user)
...
def total_dw(request):
    """Get total deposit/withdraw summary for hierarchy."""
    user = request.user
    hierarchy_users = get_hierarchy_users(user)
...
@permission_classes([IsAuthenticated])
@super_required
def master_dw(request):
    """Get deposit/withdraw by Master hierarchy."""
    user = request.user
    
    if user.role == 'POWERHOUSE':
        masters = User.objects.filter(role='MASTER')
    else:
        masters = User.objects.filter(role='MASTER', parent=user)
    
    data = []
    for master in masters:
        # Get all users under this master
        master_children = User.objects.filter(parent=master)
        
        deposits = ClientRequest.objects.filter(
            user__in=master_children, request_type='DEPOSIT', status='APPROVED'
        ).aggregate(total=Sum('amount'))['total'] or 0
        
        withdrawals = ClientRequest.objects.filter(
            user__in=master_children, request_type='WITHDRAW', status='APPROVED'
        ).aggregate(total=Sum('amount'))['total'] or 0
        ...
        data.append({
            ...
            'children_count': master_children.count(),
        })
