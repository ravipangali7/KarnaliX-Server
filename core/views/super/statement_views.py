...
def _to_statement_row(tx):
    ...
    if getattr(tx, "processed_by", None) and tx.processed_by_id:
        description = description + (" | Processed by: " + tx.processed_by.username)
    ref = getattr(tx, "reference_id", None) or ""
    return {
        "id": tx.id,
