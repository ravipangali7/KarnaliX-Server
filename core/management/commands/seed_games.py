"""
Seed games from provider list (name + provider_game_uid).
...
# Tab-separated: Game Name\tprovider_game_uid
GAMES_RAW = """Royal Fishing	e794bf5717aca371152df192341fe68b
...
def infer_game_type(name: str) -> str:
    ...
    n = name.upper()
    if "BINGO" in n:
        return Game.GameType.CASINO
    if any(x in n for x in ("TEEN PATTI", "TEENPATTI", "RUMMY", "CALLBREAK", "BLACKJACK", "BACCARAT", "POKER", "ANDAR BAHAR", "LUDO", "TONGITS", "PUSOY")):
        return Game.GameType.LIVE
    if "CRASH" in n or "HILO" in n or "LIMBO" in n or "PLINKO" in n or "MINES" in n or "TOWER" in n:
        return Game.GameType.CRASH
    if any(x in n for x in ("CRICKET", "WORLD CUP", "FOOTBALL", "SABONG", "GOAL", "TOUCHDOWN", "PENALTY KICK")):
        return Game.GameType.SPORTS
    if "ROULETTE" in n or "SIC BO" in n or "KENO" in n or "7UP7DOWN" in n or "BIG SMALL" in n or "NUMBER KING" in n:
        return Game.GameType.CASINO
    return Game.GameType.SLOT


class Command(BaseCommand):
    help = "Seed games from embedded list (name + provider_game_uid). Idempotent."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default="JILI",
            help="GameProvider code to use (default: JILI)",
        )
        parser.add_argument(
            "--provider-name",
            type=str,
            default="JILI",
            help="GameProvider name (default: JILI)",
        )
        parser.add_argument(
            "--api-endpoint",
            type=str,
            default="https://allapi.online/launch_game_js",
            help="Provider launch URL",
        )
        parser.add_argument(
            "--api-secret",
            type=str,
            default="",
            help="Provider API secret for AES encryption (e.g. from provider)",
        )
        parser.add_argument(
            "--api-token",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        api_secret = (options.get("api_secret") or "").strip()
        ...
        provider, _ = GameProvider.objects.get_or_create(
            ...
            defaults={
                ...
                "status": GameProvider.Status.ACTIVE,
            },
        )
        update_fields = []
        ...
        for line in lines:
            ...
            if len(parts) != 2:
                self.stdout.write(self.style.WARNING(f"Skipping bad line: {line[:60]}..."))
                continue
            ...
            game, was_created = Game.objects.update_or_create(
                provider=provider,
                provider_game_uid=uid,
                defaults={
                    "name": name,
                    "game_type": game_type,
                    "min_bet": Decimal("10.00"),
                    "max_bet": Decimal("100000.00"),
                    "rtp": Decimal("96.00"),
